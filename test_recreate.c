/* Recreate 1080-while-live then 1440, through lsfg-vk isolated+external.
 * Agent-runnable stand-in for RE2 windowed→FS. No Steam.
 */
...
static int pick_9070(void) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (p.vendorID == 0x1002 && p.deviceID == 0x7550) {
            phys = pd[i];
            fprintf(stderr, "phys: %s\n", p.deviceName);
            return 0;
        }
    }
    fprintf(stderr, "FAIL no 9070 XT (1002:7550)\n");
    return 1;
}
...
int main(void) {
    ...
    ai.pApplicationName = "lsfg-recreate";
    ...
    if (!createHeadless) { fprintf(stderr, "FAIL no headless surface\n"); return 1; }
