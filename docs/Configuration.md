...
- **Transport / `transport`**: Transport mechanism for one-way external presentation.
  * **`shm`** (default): POSIX shared memory CPU copy. Uses a lightweight worker thread (`CopyHop`) to walk pixels into a shared memory ring, then copies into the doubler staging buffer. Delivers **~2–6 ms latency** and is the recommended mode for live frame doubling.
  * **`dmabuf`**: DRM PRIME zero-copy DMA-BUF export. Transfers frame buffers directly between GPUs via PCIe hardware DMA without CPU mediation. *Note:* On current Linux DRM drivers (amdgpu), PRIME implicit-sync (`dma_resv`) parks the buffer for **~28–38 ms** after the hardware DMA copy completes before userspace can acquire it.
  * **`decoupled`** (or `decoupled_dma`): Decoupled zero-copy hardware DMA. Binds images to shared host-memory allocations (`p`) via `VK_EXT_external_memory_host`, bypassing cross-GPU GEM `dma_resv` coupling entirely. Provides zero CPU framebuffer memcpy and eliminates cross-device implicit sync stalls.
- **Socket / `socket`**: Path to the IPC Unix domain socket used between the layer and `lsfg-vk-app`. (Default: `~/.local/state/lsfg-vk/app.sock`, fallback: `${XDG_RUNTIME_DIR}/lsfg-vk/app.sock`). Both processes share this socket automatically.
- **Fake Swapchain / `fake_swapchain`**: When enabled, the layer provides an isolated/virtual swapchain to the application. Essential for headless workloads or games that perform dynamic windowed-to-fullscreen transitions. (Default: `false`)
- **Layer Shell / `layer_shell`**: When enabled on Wayland, `lsfg-vk-app` binds `zwlr_layer_shell_v1` for exclusive overlay presentation. (Default: `false`)
- **HUD / `hud`**: Controls whether the real-time top-left telemetry overlay is displayed by `lsfg-vk-app`. (Default: `true`)
- **Debug / `debug`**: Enables verbose debug logging for the profile. (Default: `false`)
