#!/usr/bin/env bash
# Wayland-backend one-way frame-doubling demo (9070 XT render / 9060 XT doubler+display).
# Usage: run-demo-wayland.sh
set -u
EV="$(dirname "$(readlink -f "$0")")"
APP=/home/archerc/code/lsfg-vk/build/lsfg-vk-app/lsfg-vk-app
CONF=$HOME/.config/lsfg-vk/conf.toml

# 1. start the doubler app (native Wayland backend, verbose)
LSFGVK_CONFIG=$CONF nohup $APP --profile app-oneway --session wayland -v \
    > "$EV/app.log" 2>&1 &
echo $! > "$EV/app.pid"
sleep 3
...
# 4. SIGINT the app and time the exit
python3 - "$EV" <<'PYEOF'
...
with open(os.path.join(ev, "shutdown-ms.txt"), "w") as f:
    f.write(f"Wayland sigint->exit: {ms:.0f} ms\n")
