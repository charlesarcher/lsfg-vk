...
// task-4 layer, holds per-stream state, opens real cross-device
// frame-generation contexts via the lsfg-vk backend, and (task 8) presents the
// generated + real captured frames on an output swapchain on the processing GPU.
// this only needs a real graphical vk::Vulkan on the processing device plus a
// backend Instance so it can negotiate, import staging, open a context, and
// build the output swapchain.
    ...
    try {
        ...
        try {
            vk.emplace(
                ...
                true, // isGraphical (app owns the output swapchain on this device;
