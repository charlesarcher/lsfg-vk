...
namespace lsfgvk::gui {
    ...
    int runGui(int argc, char** argv) {
        ...
        while (!glfwWindowShouldClose(window) && !g_guiState.shouldClose.load()) {
            ...
            // Card 6: Linux & Dual-GPU Options (lsfg-vk)
            ImGui::BeginChild("CardLinux", ImVec2(0, 240), true, ImGuiWindowFlags_NoScrollbar);
            ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.0f, 1.0f), "Linux & Dual-GPU Options (lsfg-vk)");
            ImGui::Separator();
            ImGui::Spacing();
            ...
            const char* presentations[] = { "Internal (Single GPU)", "External (One-Way Dual-GPU app)" };
            ImGui::Combo("Presentation", &g_guiState.presentationMode, presentations, IM_ARRAYSIZE(presentations));

            char sockBuf[256];
            std::strncpy(sockBuf, g_guiState.socketPath.c_str(), sizeof(sockBuf) - 1);
            sockBuf[sizeof(sockBuf) - 1] = '\0';
            if (ImGui::InputText("Socket Path", sockBuf, sizeof(sockBuf))) {
                g_guiState.socketPath = sockBuf;
            }

            ImGui::Checkbox("Use Wayland Layer-Shell (exclusive)", &g_guiState.useLayerShell);
            ImGui::Checkbox("Emulate Isolated Swapchain (fake)", &g_guiState.fakeSwapchain);
            ImGui::Checkbox("Show Telemetry HUD", &g_guiState.showHud);
            ImGui::EndChild();

            ImGui::Spacing();

            // Card 7: Profile & Action
